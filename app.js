  ...
  document.getElementById('bk-barbers-list').innerHTML =
    ...
    eligible.map(b=>`
      <div class="bk-barber-card ${DB.booking.barber===b.id?'selected':''}" onclick="selectBarber('${b.id}',this)" data-id="${b.id}">
        ...
        <div class="bk-barber-spec">${b.specialty||'Barbeiro'}</div>
      </div>`).join('');
}

function selectBarber(id, el) {
  DB.booking.barber = id;
...
function renderBkCalendar() {
  ...
  document.getElementById('bk-calendar').innerHTML = availableDates.map(d=>`
    <button class="bk-date-btn" onclick="selectDate('${d}',this)" data-date="${d}">${new Date(d+'T00:00:00').toLocaleDateString('pt-BR', {month:'short',day:'numeric'})}</button>`).join('');
}

function selectDate(date, el) {
  DB.booking.date = date;
  document.querySelectorAll('.bk-date-btn').forEach(b=>b.classList.remove('selected'));
  el.classList.add('selected');
  renderBkTimes();
}

function renderBkTimes() {
  if (!DB.booking.date) return;
  const barber = DB.booking.barber ? DB.barbers.find(b=>b.id===DB.booking.barber) : null;
  const times = [];
  for (let h = 9; h <= 18; h++) {
    for (let m of [0, 30]) {
      const t = `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
      const occupied = DB.appointments.some(a => a.date === DB.booking.date && a.time === t && (!barber || a.barber === barber.id));
      if (!occupied) times.push(t);
    }
  }
  document.getElementById('bk-times').innerHTML = times.map(t=>`
    <button class="bk-time-btn" onclick="selectTime('${t}',this)" data-time="${t}">${t}</button>`).join('');
}

function selectTime(time, el) {
  DB.booking.time = time;
  document.querySelectorAll('.bk-time-btn').forEach(b=>b.classList.remove('selected'));
  el.classList.add('selected');
}

function goStep(step) {
  currentStep = step;
  document.querySelectorAll('[id^="bk-step-"]').forEach(e=>e.style.display='none');
  document.getElementById('bk-step-' + step).style.display='block';
  updateStepButtons();
}

function updateStepButtons() {
  const btn1 = document.getElementById('bk-btn-prev');
  const btn2 = document.getElementById('bk-btn-next');
  if (btn1) btn1.style.display = currentStep > 1 ? 'block' : 'none';
  if (btn2) btn2.style.display = currentStep < 4 ? 'block' : 'none';
  if (currentStep === 4 && document.getElementById('bk-btn-confirm')) {
    document.getElementById('bk-btn-confirm').style.display='block';
  }
}
...
function confirmBooking() {
  ...
  DB.appointments.push({
    ...
    barber: DB.booking.barber,
    date: DB.booking.date,
    time: DB.booking.time,
    status: 'agendado'
  });
  
  alert('Agendamento confirmado! ' + (DB.identity.welcome || 'Obrigado!'));
  initBooking();